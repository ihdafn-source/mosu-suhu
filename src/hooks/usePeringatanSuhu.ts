...
export function usePeringatanSuhu(suhu: number | null, threshold = 27) {
  return useMemo(() => {
    ...
    if (suhu <= threshold * 0.9) return { status: "normal" as StatusSuhu, message: "Suhu dalam rentang normal." };
    if (suhu <= threshold) return { status: "waspada" as StatusSuhu, message: `Suhu mendekati batas (${threshold}°C), waspada!` };
